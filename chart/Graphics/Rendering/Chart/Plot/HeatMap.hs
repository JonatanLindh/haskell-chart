...
plotHeatMap phm =
  Plot
    ...
    , _plot_legend = [(_plot_heatmap_title phm, const(return () )), ("-1", const(return () )) ,(" ", renderPlotLegendHeatMap (-1, 1) phm), ("1", const(return () )) ]
...
renderPlotLegendHeatMap :: (Double, Double) -> PlotHeatMap x y z -> Rect -> BackendProgram ()
